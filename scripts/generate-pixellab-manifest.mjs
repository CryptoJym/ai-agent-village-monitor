...
const INTERIORS_ROOT = path.join(PUBLIC_ROOT, 'interiors');
...
async function collectInteriors() {
  const result = {};
  if (!(await pathExists(INTERIORS_ROOT))) {
    return result;
  }
  const themes = await fs.readdir(INTERIORS_ROOT, { withFileTypes: true });
  for (const themeDir of themes) {
    if (!themeDir.isDirectory()) continue;
    const themeKey = themeDir.name;
    const metadataPath = path.join(INTERIORS_ROOT, themeKey, 'metadata.json');
    if (!(await pathExists(metadataPath))) continue;
    try {
      const raw = await fs.readFile(metadataPath, 'utf8');
      result[themeKey] = JSON.parse(raw);
    } catch (err) {
      if (process.env.DEBUG_PIXELLAB_MANIFEST === 'true') {
        console.warn(`Failed to parse interior metadata for ${themeKey}:`, err);
      }
    }
  }
  return result;
...
async function buildManifest() {
  ...
  const interiorManifest = await collectInteriors();
