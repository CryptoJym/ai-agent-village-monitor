...
export const DEFAULT_UPSTREAM_SOURCES = [
    {
...
export class VersionWatcher extends EventEmitter {
     ...
     */
    async fetchFromNpm(source) {
        const controller = new AbortController();
        const timeout = setTimeout(() => controller.abort(), this.config.httpTimeoutMs);
        try {
            const response = await fetch(`https://registry.npmjs.org/${source.source}/latest`, {
    ...
    async fetchFromGitHub(source) {
        ...
        const timeout = setTimeout(() => controller.abort(), this.config.httpTimeoutMs);
        try {
            const response = await fetch(`https://api.github.com/repos/${source.source}/releases/latest`, {
                signal: controller.signal,
                headers: {
     ...
     */
    async fetchFromHomebrew(source) {
        const controller = new AbortController();
        const timeout = setTimeout(() => controller.abort(), this.config.httpTimeoutMs);
        try {
            const response = await fetch(`https://formulae.brew.sh/api/formula/${source.source}.json`, {
                signal: controller.signal,
                headers: { Accept: 'application/json' },
            });
            if (!response.ok) {
     ...
     */
    registerHeartbeatVersion(providerId, version) {
