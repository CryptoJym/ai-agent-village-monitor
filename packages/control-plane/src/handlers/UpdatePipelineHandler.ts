...
const DEFAULT_CONFIG: UpdatePipelineHandlerConfig = {
  enableCaching: true,
...
export class UpdatePipelineHandler extends EventEmitter {
  private config: UpdatePipelineHandlerConfig;
  private pipeline: UpdatePipeline | null = null;
  private versionCache: Map<string, CacheEntry<VersionInfo[]>> = new Map();
  private buildCache: Map<string, CacheEntry<BuildInfo>> = new Map();
   ...
   */
  async getBuild(buildId: string): Promise<ApiResponse<BuildInfo>> {
    const requestId = uuidv4();
    ...
    try {
      ...
      if (filters?.providerId) {
        builds = builds.filter(b =>
          Object.keys(b.runtimeVersions).includes(filters.providerId!)
        );
      }
   ...
   * Get active rollouts.
   */
  async getActiveRollouts(): Promise<ApiResponse<RolloutStatus[]>> {
    const requestId = uuidv4();

    try {
      const rollouts = await this.fetchActiveRollouts();

      const statuses: RolloutStatus[] = rollouts.map(r => this.toRolloutStatus(r));
   ...
   */
  async getRollout(rolloutId: string): Promise<ApiResponse<RolloutStatus>> {
    const requestId = uuidv4();
  ...
  async startRollout(
    ...
    try {
      ...
      const rollout: ActiveRollout = {
        ...
        channel,
        state: 'in_progress' as RolloutState,
        currentPercentage: 0,
   ...
   */
  async pauseRollout(rolloutId: string): Promise<ApiResponse<RolloutStatus>> {
    const requestId = uuidv4();
    ...
    try {
      ...
      const mutableRollout = rollout as { state: RolloutState; lastUpdatedAt: Date };
   ...
   */
  async resumeRollout(rolloutId: string): Promise<ApiResponse<RolloutStatus>> {
    const requestId = uuidv4();
  ...
  async rollbackRollout(
    ...
    try {
      ...
      // Update state
      const mutableRollout = rollout as { state: RolloutState; lastUpdatedAt: Date; error?: string };
   ...
   */
  async getKnownGoodBuilds(
    providerId: ProviderId
  ): Promise<ApiResponse<BuildInfo[]>> {
    const requestId = uuidv4();

    try {
      const entries = await this.fetchKnownGoodEntries(providerId);

      const buildInfos: BuildInfo[] = entries.map(e => ({
        buildId: e.buildId,
  ...
  private async fetchVersions(
    ...
    _channel?: 'stable' | 'beta'
  ): Promise<RuntimeVersion[]> {
    // Would query VersionWatcher
    return [];
  }

  private async fetchBuild(_buildId: string): Promise<RunnerBuild | null> {
    // Would query KnownGoodRegistry
    return null;
  }

  private async fetchAllBuilds(): Promise<RunnerBuild[]> {
    // Would query KnownGoodRegistry
    return [];
  }

  private async fetchActiveRollouts(): Promise<ActiveRollout[]> {
    // Would query RolloutController
    return [];
  }

  private async fetchRollout(_rolloutId: string): Promise<ActiveRollout | null> {
    // Would query RolloutController
    return null;
  }

  private async fetchKnownGoodEntries(
    _providerId: ProviderId
  ...
  private toBuildInfo(build: RunnerBuild): BuildInfo {
    return {
  ...
  private toRolloutStatus(rollout: ActiveRollout): RolloutStatus {
    return {
      rolloutId: rollout.rolloutId,
