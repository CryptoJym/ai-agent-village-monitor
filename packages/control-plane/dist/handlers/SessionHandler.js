...
export class SessionHandler extends EventEmitter {
     ...
     */
    async listSessions(orgId, pagination, filters) {
        const requestId = uuidv4();
     ...
     */
    requestApproval(sessionId, action, description, context) {
        const session = this.sessions.get(sessionId);
        if (!session) {
            return null;
        }
     ...
     */
    async resolveApproval(sessionId, request) {
        const requestId = uuidv4();
        const session = this.sessions.get(sessionId);
        if (!session) {
            return {
        ...
        const approvalIndex = session.pendingApprovals.findIndex(a => a.approvalId === request.approvalId);
        if (approvalIndex === -1) {
            return {
                success: false,
                error: {
