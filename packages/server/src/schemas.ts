...
export const AgentCommandSchema = z.union([
  ...
  z.object({ command: z.literal('run_tool'), args: z.object({ tool: z.string().min(1) }).passthrough() }),
  // Run tool with top-level tool
  z.object({ command: z.literal('run_tool'), tool: z.string().min(1), args: z.record(z.any()).optional() }),
  z.object({ command: z.literal('commit'), message: z.string().min(1) }),
  z.object({ command: z.literal('pull_request'), title: z.string().min(1), body: z.string().optional(), draft: z.boolean().optional() }),
