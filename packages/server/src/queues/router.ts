...
queuesRouter.get('/queues/agent-commands/dlq', async (_req, res) => {
  try {
    ...
    const out = jobs.map((j: any) => ({ id: j.id, name: j.name, data: j.data, failedReason: (j as any).failedReason }));
...
queuesRouter.post('/queues/agent-commands/dlq/requeue', validateBody(RequeueSchema), async (req, res) => {
  try {
    ensureRedis();
    // eslint-disable-next-line @typescript-eslint/no-var-requires
    const { Queue } = require('bullmq');
    ...
    await main.add(job.name, job.data, { removeOnComplete: { count: 500, age: 3600 }, removeOnFail: { age: 24 * 3600 } });
    await job.remove();
...
queuesRouter.delete('/queues/agent-commands/dlq', validateBody(DlqDeleteBodySchema), validateQuery(DlqDeleteQuerySchema), async (req, res) => {
  try {
    ensureRedis();
    // eslint-disable-next-line @typescript-eslint/no-var-requires
    const { Queue } = require('bullmq');
    const dlq = new Queue('agent-commands-dlq', { connection: getRedis()! });
    const jobId = (req as any).validatedBody?.jobId ? String((req as any).validatedBody.jobId) : '';
    if (jobId) {
      const job = await dlq.getJob(jobId);
      if (!job) return res.status(404).json({ error: 'job not found', code: 'NOT_FOUND' });
      await job.remove();
    ...
    if ((req as any).validatedQuery?.all !== 'true') return res.status(400).json({ error: 'missing all=true to purge', code: 'BAD_REQUEST' });
