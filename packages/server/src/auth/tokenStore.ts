...
export async function saveProviderToken(opts: {
  userKey: string;
  provider: Provider;
  token: string;
  scopes?: string;
  ...
  if (enc) {
    ...
    await prisma['oAuthToken']?.upsert({
      where: { userKey_provider: { userKey: opts.userKey, provider: opts.provider } },
      update: {
        encCiphertext: digest,
        encIv: Buffer.alloc(0),
        encTag: Buffer.alloc(0),
        version: 0,
        scopes: opts.scopes ?? undefined,
      },
      create: {
...
export async function getProviderToken(opts: {
  userKey: string;
  provider: Provider;
}): Promise<string | null> {
  const prisma = getPrisma();
  ...
  if (token) {
    try {
      ...
      // Ignore last-used update failures; token retrieval succeeded.
    }
  }
  return token;
}
...
export async function revokeProviderToken(opts: { userKey: string; provider: Provider }) {
  ...
  try {
    ...
    // Token already absent; treat revoke as success.
  }
}
