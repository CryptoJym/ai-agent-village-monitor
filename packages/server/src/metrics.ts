...
export function observe(name: string, value: number, labels?: Labels, buckets: number[] = [5, 10, 25, 50, 100, 250, 500, 1000, 2000]) {
  const k = keyFor(name, labels);
  let h = histograms.get(k);
  if (!h) {
    h = { sum: 0, count: 0, buckets: [...buckets], counts: new Array(buckets.length + 1).fill(0), labels: labels || {} };
    histograms.set(k, h);
  }
  h.sum += value;
  h.count += 1;
...
export function getMetrics() {
  ...
  for (const [k, h] of histograms.entries()) obj[k] = { sum: h.sum, count: h.count, buckets: h.buckets, counts: h.counts };
  return obj;
}
...
export function getPrometheus(): string {
  ...
  for (const [k, h] of histograms.entries()) {
    const [name, labelsStr] = k.split('|', 2);
    const baseLabels = labelsStr ? Object.fromEntries(labelsStr.split('|').map((p) => p.split('='))) as Labels : {};
    let cum = 0;
    for (let i = 0; i < h.counts.length; i++) {
      cum += h.counts[i];
      const le = i < h.buckets.length ? h.buckets[i] : '+Inf';
      const allLabels = { ...baseLabels, le: String(le) };
      const lbls = '{' + Object.keys(allLabels).sort().map((lk) => `${lk}="${allLabels[lk]}"`).join(',') + '}';
      lines.push(`${name}_bucket${lbls} ${cum}`);
    }
    const baseLblStr = labelsStr ? '{' + Object.keys(baseLabels).sort().map((lk) => `${lk}="${baseLabels[lk]}"`).join(',') + '}' : '';
