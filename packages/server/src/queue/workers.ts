...
export function startWorkers(log = console): WorkerHandles | null {
    ...
    try {
      if (kind === 'start') {
        ...
        if (prisma) {
          ...
          try { await prisma.agent.update({ where: { id: agentIdStr }, data: { status: 'working' } }); } catch {}
        ...
        try { const { audit } = await import('../audit/logger'); audit({ type: 'session_started', agentId: agentIdStr, sessionId, jobId: String(job.id), ts: Date.now() }); } catch {}
        emitToAgent(agentIdStr, 'agent_update', { agentId: agentIdStr, status: 'working', ts: Date.now() });
        emitToAgent(agentIdStr, 'work_stream', { agentId: agentIdStr, message: 'session started', ts: Date.now() });
        ...
        if (prisma) {
          ...
          try { await prisma.agent.update({ where: { id: agentIdStr }, data: { status: 'idle' } }); } catch {}
        }
        try { const { audit } = await import('../audit/logger'); audit({ type: 'session_stopped', agentId: agentIdStr, jobId: String(job.id), ts: Date.now() }); } catch {}
        emitToAgent(agentIdStr, 'work_stream', { agentId: agentIdStr, message: 'session stopped', ts: Date.now() });
        emitToAgent(agentIdStr, 'agent_update', { agentId: agentIdStr, status: 'idle', ts: Date.now() });
        ...
        try { const { audit } = await import('../audit/logger'); audit({ type: 'command_enqueued', agentId: agentIdStr, sessionId, jobId: String(job.id), data: { cmd }, ts: Date.now() }); } catch {}
        audit('agent.command_started', { agentId: agentIdStr, jobId: String(job.id), command: cmd });
        emitToAgent(agentIdStr, 'work_stream', { agentId: agentIdStr, message: `command received: ${cmd}` , ts: Date.now() });
          ...
          onEvent: async (evt) => {
            try {
              if (evt.type === 'log' && evt.message) {
                emitToAgent(agentIdStr, 'work_stream', { agentId: agentIdStr, message: evt.message, ts: Date.now() });
                if (prisma && sessionId) await appendEvent(sessionId, 'log', evt.message);
              } else if (evt.type === 'progress') {
                emitToAgent(agentIdStr, 'work_stream', { agentId: agentIdStr, message: `progress: ${Math.round((evt.progress || 0) * 100)}%`, ts: Date.now(), progress: evt.progress });
                if (prisma && sessionId) await appendEvent(sessionId, 'progress', undefined, { progress: evt.progress, data: evt.data });
              } else if (evt.type === 'status') {
                emitToAgent(agentIdStr, 'agent_update', { agentId: agentIdStr, status: evt.message || 'working', ts: Date.now() });
                if (prisma && sessionId) await appendEvent(sessionId, 'status', evt.message);
              } else if (evt.type === 'error') {
                emitToAgent(agentIdStr, 'work_stream', { agentId: agentIdStr, message: evt.message || 'error', ts: Date.now() });
                if (prisma && sessionId) await appendEvent(sessionId, 'error', evt.message, evt.data);
              }
            } catch {}
          },
        } as any;
        ...
        if (prisma && sessionId) {
          await appendEvent(sessionId, result.ok ? 'command_completed' : 'command_failed', result.ok ? 'ok' : (result.error || 'error'), result);
        }
        try { const { audit } = await import('../audit/logger'); audit({ type: result.ok ? 'command_completed' : 'command_failed', agentId: agentIdStr, sessionId, jobId: String(job.id), data: { cmd, result: result.ok ? 'ok' : result.error }, ts: Date.now() }); } catch {}
        emitToAgent(agentIdStr, 'work_stream', { agentId: agentIdStr, message: result.ok ? `command completed: ${cmd}` : `command failed: ${cmd}`, ts: Date.now() });
        audit(result.ok ? 'agent.command_completed' : 'agent.command_failed', { agentId: agentIdStr, jobId: String(job.id), command: cmd, ok: result.ok });
      }
    } catch (e: any) {
      log.error?.('[worker] agent-commands error', { jobId: job.id, err: e?.message });
      try { audit('agent.command_error', { agentId: agentIdStr, jobId: String(job.id), error: e?.message }, 'error'); } catch {}
      throw e;
    }
  ...
  const syncProc: Processor = async (job) => {
    ...
    if (!Number.isFinite(villageId) || villageId <= 0 || !org) throw new Error('invalid job payload');
  ...
  agentCommands.on('failed', async (job, err) => {
    ...
    try {
      ...
      if (made >= attempts) {
        await dlq.add('failed', { name: job?.name, data: job?.data, failedReason: err?.message }, { removeOnComplete: 1000, removeOnFail: 1000 });
        const { emitToAgent } = await import('../realtime/io');
        const agentId = String((job?.data as any)?.agentId ?? 'unknown');
        emitToAgent(agentId, 'work_stream', { agentId, message: `command failed permanently: ${job?.name}`, error: err?.message, ts: Date.now() });
        try { audit('agent.command_dlq', { agentId, jobId: String(job?.id), name: job?.name, error: err?.message }, 'error'); } catch {}
      }
    } catch {}
  });
  githubSync.on('failed', (job, err) => log.error?.('[worker] github-sync failed', { jobId: job?.id, err: err?.message }));
