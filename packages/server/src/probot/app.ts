...
export function registerProbotHandlers(app: any) {
    ...
    app.on('check_run.completed', async (context: any) => {
      ...
      try {
        ...
        const failed = ['failure', 'timed_out', 'action_required', 'cancelled', 'stalled'].includes(conclusion);
...
export async function getProbotMiddleware() {
  ...
  const secret = (process.env.GITHUB_WEBHOOK_SECRET || process.env.WEBHOOK_SECRET) as string | undefined;
  if (!appId || !privateKey || !secret) {
    throw new Error('Probot env missing (GITHUB_APP_ID, GITHUB_PRIVATE_KEY, GITHUB_WEBHOOK_SECRET)');
  }
