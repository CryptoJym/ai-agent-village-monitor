...
describe('role-based visibility and flows', () => {
  ...
  beforeAll(async () => {
    ...
    member = await prisma.user.create({
      data: { githubId: BigInt(ts + 1), username: `member_${ts}` },
    });
    ...
    await prisma.villageAccess.create({
      data: { villageId: v.id, userId: member!.id, role: 'member' },
    });
  });
  ...
  it.skipIf(!hasDb)(
    'owner can toggle public; unauthenticated sees viewerRole=visitor',
    async () => {
