...
function ms(v) { return Number(v) || 0; }
...
async function main() {
  ...
  const csvPath = csvFlag ? (csvFlag === 'true' ? (args['csv-path'] || 'load-metrics.csv') : csvFlag) : '';
  const csvAppend = args['csv-append'] === 'true' || args['csv-append'] === '1';

  if (!jwtSecret) {
    console.warn('[load] No JWT secret provided; connections will be rejected if server enforces auth.');
  }

  console.log(`[load] target=${url} clients=${clients} ramp=${ramp}s time=${time}s transport=${transport.join(',')}`);
  ...
  function connectOne(i) {
    ...
    socket.on('connect', async () => {
      ...
      try {
        if (village) await new Promise((resolve) => socket.timeout(2000).emit('join_village', { villageId: String(village) }, () => resolve(undefined)));
        if (repo) await new Promise((resolve) => socket.timeout(2000).emit('join_repo', { repoId: String(repo) }, () => resolve(undefined)));
      } catch {}
      // Start periodic RTT checks per client
      const tick = () => {
        const start = Date.now();
        try {
          socket.timeout(2000).emit('ping', () => { rtts[i] = Date.now() - start; });
        } catch {}
      };
      tick();
  ...
  const print = setInterval(() => {
    ...
    loop.reset();
    console.log(`[load] t=${secs}s conns=${connected.size}/${clients} fail=${failed.size} p50=${p50}ms p95=${p95}ms evloop.max=${lag}ms mem.rss=${rssMB}MB`);
    metrics.push({
  ...
  setTimeout(() => {
    ...
    console.log(`[load] DONE conns=${connected.size}/${clients} fail=${failed.size} p50=${p50}ms p95=${p95}ms evloop.max=${lag}ms`);
    if (csvPath) {
      const fs = require('fs');
      const header = 'timestamp,t_s,conns,clients,fail,p50_ms,p95_ms,evloop_max_ms,mem_rss_mb,transport\n';
      const rows = metrics.map(m => `${m.timestamp},${m.t_s},${m.conns},${m.clients},${m.fail},${m.p50_ms},${m.p95_ms},${m.evloop_max_ms},${m.mem_rss_mb},${m.transport}`).join('\n') + '\n';
      try {
